...
function (_React$Component) {
  ...
  _createClass(HTMLEllipsis, [{
    ...
    key: "hookNode",
    value: function hookNode(node, basedOn) {
      var _this2 = this;
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        var frag = this.props.window.document.createDocumentFragment();
        var units;
        ...
        units.forEach(function (unit) {
          frag.appendChild(_this2.dummySpan(unit));
        });
    ...
    key: "dummySpan",
    value: function dummySpan(text) {
      var span = this.props.window.document.createElement('span');
    ...
    key: "unwrapTextNode",
    value: function unwrapTextNode(node) {
      node.parentNode.replaceChild(this.props.window.document.createTextNode(node.textContent), node);
    }
  }, {
    key: "removeFollowingElementLeaves",
    value: function removeFollowingElementLeaves(node, root) {
    ...
    key: "findBlockAncestor",
    value: function findBlockAncestor(node) {
      var ndAncestor = node;
    ...
    key: "affectLayout",
    value: function affectLayout(ndUnit) {
    ...
    value: function initCanvas() {
      ...
      var canvas = this.canvas = this.props.window.document.createElement('div');
    ...
    value: function copyStyleToCanvas() {
      ...
      mirrorProps.forEach(function (key) {
        _this3.canvas.style[key] = targetStyle[key];
      });
    }
  }, {
    key: "reflow",
    ...
    value: function calcIndexes() {
      ...
      var nd1stLayoutUnit = nlUnits.find(this.affectLayout);
    ...
    value: function putEllipsis(indexes) {
      ...
      } while (ndPrevUnit && (!this.affectLayout(ndPrevUnit) || ndEllipsis.offsetHeight > ndPrevUnit.offsetHeight || ndEllipsis.offsetTop > ndPrevUnit.offsetTop));
    ...
    value: function makeEllipsisSpan() {
      ...
      var frag = this.props.window.document.createElement('span');
      frag.appendChild(this.props.window.document.createElement('wbr'));
      var ndEllipsis = this.props.window.document.createElement('span');
